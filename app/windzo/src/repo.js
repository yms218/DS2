export async function reloadRepoCards() {
  const CACHE_TIMEOUT = 0; // 3시간
  ...
  async function get(url) {
    ...
    try {
      ...
      return prevResp ? prevResp.data : null;
    }
  }

  const emojis = await get('http://localhost:8000/github/emojis');
  const colors = await get('http://localhost:8000/github/colors');
  ...
  for (const el of document.querySelectorAll('.repo-card')) {
    ...
    const data = await get(`http://localhost:8000/github/repos/${owner}/${repo}`);
    ...
    data.description = (data.description || '').replace(/:\w+:/g, function(match) {
      ...
      if (emoji) {
        return `<span class="emoji"><img src="${emoji}" alt="${name}"></span>`;
      }
